...
class VideoLLAMA(Blip2Base):
    ...
    def encode_videoQformer_visual(self, image, output_video_embedding=False, output_llama_embedding=False):
        device = image.device
    ...
    def forward(self, samples):
        if 'conv_type' in samples.keys() and samples['conv_type']=='multi':
            ...
            if self.use_clip_loss:
                ...
                clip_loss = self.compute_clip_loss(clip_image_feature, clip_text_feature)
            #exit()
